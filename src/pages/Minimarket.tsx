...
import { ShoppingCart, Trash2, CreditCard, QrCode, Printer } from 'lucide-react';
    ...
    <>
      ...
      <div className="mb-3">
        ...
        <Select value={selectedMember} onValueChange={setSelectedMember}>
          <SelectTrigger><SelectValue placeholder="Tanpa anggota" /></SelectTrigger>
          <SelectContent>
            <SelectItem value="none">Tanpa anggota</SelectItem>
            {members.map((m: any) => <SelectItem key={m.id} value={String(m.id)}>{m.nama}</SelectItem>)}
          </SelectContent>
        </Select>
      </div>
      ...
      <Button onClick={handleCheckout} disabled={cart.length === 0} className="w-full h-12 text-[15px] font-bold gap-2">
...
export default function Minimarket() {
  ...
  return (
    <div className="flex h-full overflow-hidden relative">
      ...
      <div className="flex-1 flex flex-col p-4 sm:p-5 sm:pl-7 overflow-hidden">
        ...
        <div className="flex gap-2 mb-3">
          ...
          <Button
            ...
            size="icon"
            className="sm:w-auto sm:px-3 sm:gap-1.5"
          >
            <QrCode className="h-4 w-4" />
            <span className="hidden sm:inline">{scanMode ? 'Scan Aktif' : 'Scan'}</span>
          </Button>
        </div>
      ...
      {!isMobile && (
        <div className="w-80 min-w-[280px] bg-card border-l flex flex-col p-5">
      ...
      <Dialog open={qrisOpen} onOpenChange={setQrisOpen}>
        <DialogContent className="sm:max-w-md max-h-[90vh] overflow-y-auto">
          ...
          <div className="text-center">
            <div className="text-xl sm:text-2xl font-bold text-primary mb-4">{formatRp(cartTotal)}</div>
            ...
            <img
              ...
              className="max-h-[320px] sm:max-h-[420px] w-auto max-w-full rounded-xl border bg-card mx-auto mb-4 object-contain"
              loading="lazy"
            />
            <p className="text-xs sm:text-sm text-muted-foreground mb-4">Scan QR code di atas menggunakan e-wallet atau mobile banking</p>
            <Button variant="outline" className="w-full mb-2 gap-2" onClick={() => printStruk(pendingStruk)}>
              <Printer className="h-4 w-4" />
              Cetak Nota
            </Button>
          </div>
          <DialogFooter className="flex gap-2 sm:gap-2">
            <Button variant="outline" className="flex-1" onClick={() => setQrisOpen(false)}>Batal</Button>
            <Button className="flex-1" onClick={confirmQris}>✅ Sudah Bayar</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
